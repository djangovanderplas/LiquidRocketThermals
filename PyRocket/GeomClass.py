...
class ChamberGeometry():
    ...
    """
    def __init__(self, D_c, D_t, D_e, L_cyl, r_1, r_2, r_n, phi_conv, phi_div, phi_e, step_size=0.001, contour_path=None):
        ...
        if contour_path is None:
            self.contour_path = Path(__file__).resolve().parent.parent / "data" / "Contour.csv"
        else:
            self.contour_path = Path(contour_path)
    ...
    def contour(self):
        ...
        if not self.contour_path.exists():
            raise FileNotFoundError(f"Contour file not found: {self.contour_path}")

        xy = np.genfromtxt(self.contour_path, delimiter=',')
    ...
    def plot_contour(self, path=False):
        ...
        if path != False:
            ...
            plt.savefig(save_path / "chamber_contour.png", dpi=400)
            plt.close()
        else:
            plt.show()
...
class CoolingGeometry():
    ...
    This is meant for program validation purposes and can be disabled in config.toml.
    """
