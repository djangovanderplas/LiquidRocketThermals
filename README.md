...
## Installation
PyRocket relies on Python dependencies plus GMSH for meshing. Recent `rocketcea` wheels ship with precompiled CEA binaries on common platforms, so a local FORTRAN compiler is not required for typical installs.


### Installation
Install Python and then install the package dependencies.

```
    1. install Python (3.11+ recommended)
    2. clone PyRocket files into a folder
...
## Verify Installation
In order to verify that all pacakges were installed correctly, run the section solver module from the repository root.
```bash
    python3 -m PyRocket.SectionThermalSim
```
This will run a single transient thermal simulation. Do not alter the contents of the config.toml file before running. The expected ouput is the maximum temperature
at every time step printed in the terminal window. Additionally a directory called 'TestSectionThermalSim' is created under `outputs/`, where an image of the temperature profile
of the test section is saved. This unit test can be repeated after altering the config.toml file for each use case to verify the stability of your new time step and 
mesh resolution settings. 

Unit tests are located under `tests/unit/` and can be run directly, e.g.:
```bash
    python3 tests/unit/test_unit.py
```
...
```bash
    python3 run.py
```
After installing with `pip`, you can also run `pyrocket-run`.

By default, simulation outputs are written to `outputs/SectionImages`, and the chamber contour CSV is loaded from `data/Contour.csv`.
`data/Contour.csv` is committed as an example contour file and can be replaced with your chamber profile.

### Debug mode
Set the 'print_result' option to True in config.toml in order to display more detialed information on the each transient thermal simulation. The output will show each time 
...
The combustion chamber is defined using a series of parameters in config.toml that describe the cylindrical chamber section, and the converging and diverging sections 
...
exchanged in this program. A plot of the chamber contour is saved in the target directory before each run. The variable 'step_size' in config.toml determines the 
...
Changes to the mesh can be verified by running `python3 -m PyRocket.SectionThermalSim`.
...
or after a set maximum number of iterations is reached. All settings for the 2D thermal sim are found in config.toml. During a full run of the program, the boundary flux over
...
Four common rocket engine chamber matierals can be selected from MaterialLib.py, 'Ti-6Al-4V', 'IN718', 'CuCr1Zr' and 'SS 1.4404'. These can be selected in config.toml.
...
In config.toml any fuel and oxidiser present in [RocketCEA](https://rocketcea.readthedocs.io/en/latest/) can be input. If a new propellant is desired, it can be 
...
the correct Reynolds number in the channels. By setting 'start_idx' in config.toml to 0 the flow of coolant starts at the injector. Run this simulation. 
...
For Validation purposes the axial and radial coordinate of thermocouples along the camber can be set in config.toml. The temperature at that location will then be displayed in the 
